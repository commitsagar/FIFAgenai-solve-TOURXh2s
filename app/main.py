...
# --- THREAD-SAFE IN-MEMORY CACHE FOR PERFORMANCE & SCALABILITY ---
class QueryCache:
    ...
    def set(self, session_id: str, query: str, response: str):
        key = f"{session_id}:{query.lower().strip()}"
        with self._lock:
            self._cache[key] = (response, time.time())

# Instantiate cache
chat_cache = QueryCache(expiry_seconds=300)

class ChatRequest(BaseModel):
    message: str = Field(..., min_length=1, description="Message string from visitor")
    session_id: str = Field(..., description="Unique session token ID")
    user_id: str = Field(..., description="Client user id reference")
    state: Dict[str, Any] = Field(default_factory=dict, description="Sync telemetry states")

class ConfigUpdateRequest(BaseModel):
    api_mode: str
    api_key: str

def run_local_mock_routing(message: str, state: Dict[str, Any]) -> tuple[str, str]:
    """Evaluates user requests locally using synthetic telemetry records."""
    text_lower = message.lower()
    response_text = ""
    active_agent = "coordinator"

    # 1. Check Language Switch Intent
    if any(k in text_lower for k in ["switch to", "speak in", "translate to", "change to", "language to", "மாற்று", "பேசு", "बात करो", "बोलो"]):
        target_lang = None
        lang_name = ""
        reply = ""
        
        if "spanish" in text_lower or "espanol" in text_lower or "español" in text_lower:
            target_lang, lang_name, reply = "es", "Spanish", "Perfecto, he cambiado el idioma a español. ¿En qué puedo ayudarte?"
        elif "tamil" in text_lower or "தமிழ்" in text_lower or "தமிழு" in text_lower:
            target_lang, lang_name, reply = "ta", "Tamil", "நிச்சயமாக, நான் மொழியை தமிழுக்கு மாற்றியுள்ளேன். நான் உங்களுக்கு எவ்வாறு உதவ முடியும்?"
        elif "hindi" in text_lower or "हिंदी" in text_lower or "हिन्दी" in text_lower:
            target_lang, lang_name, reply = "hi", "Hindi", "बिल्कुल, मैंने भाषा बदलकर हिंदी कर दी है। आज मैं आपकी क्या मदद कर सकता हूँ?"
        elif "english" in text_lower or "अंग्रेजी" in text_lower or "ஆங்கிலம்" in text_lower:
            target_lang, lang_name, reply = "en", "English", "Certainly! I have switched the language to English. How can I help you today?"
        elif "french" in text_lower or "français" in text_lower or "francais" in text_lower:
            target_lang, lang_name, reply = "fr", "French", "D'accord, j'ai changé la langue en français. Comment puis-je vous aider aujourd'hui?"
        elif "portuguese" in text_lower or "portugues" in text_lower:
            target_lang, lang_name, reply = "pt", "Portuguese", "Com certeza, mudei o idioma para português. Como posso te ajudar hoje?"
        elif "arabic" in text_lower or "عربي" in text_lower or "العربية" in text_lower:
            target_lang, lang_name, reply = "ar", "Arabic", "بالتأكيد، لقد قمت بتغيير اللغة إلى العربية. كيف يمكنني مساعدتك اليوم؟"
            
        if target_lang:
            state["language"] = target_lang
            return reply, "coordinator"

    # Multilingual script routing
    is_hindi = bool(re.search(r"[\u0900-\u097F]", message))
    is_tamil = bool(re.search(r"[\u0B80-\u0BFF]", message))
    ...
    if is_hindi:
        ...
        if any(k in text_lower for k in ["கழிப்பறை", "டாய்லெட்", "சக்கர"]):
            response_text = "அரங்கத்தின் அனைத்து நிலைகளிலும் மாற்றுத்திறனாளிகளுக்கான கழிப்பறைகள் உள்ளன. அருகிலுள்ள சக்கர நாற்காலை அணுகக்கூடிய கழிப்பறை மேற்கு கழிப்பறை தொகுதி (கேட் எஃப் அருகில்) மற்றும் உணவு ஹப் 1-க்கு அருகில் உள்ளது."
        ...
        if any(k in text_lower for k in ["wheelchair", "accessible", "ada", "elevat", "toilet", "restroom"]):
        ...
        elif any(k in text_lower for k in ["recycl", "sustainab", "plastic", "trash", "garbage", "bin", "can"]):
            active_agent = "fan_assistant"
            ...
            min_wait = 6
            facs = state.get("facilitiesState", {})
            ...
            scen_name = state.get("activeScenario", "standard")
            if scen_name == "storm":
        ...
        elif any(k in text_lower for k in ["speak", "understand", "support", "language"]) and any(lang in text_lower for lang in ["spanish", "tamil", "hindi", "french", "portuguese", "arabic", "english"]):
            target_lang = "en"
            lang_name = "English"
            if "spanish" in text_lower or "es" in text_lower:
                target_lang, lang_name = "es", "Spanish"
            elif "tamil" in text_lower or "ta" in text_lower:
                target_lang, lang_name = "ta", "Tamil"
            elif "hindi" in text_lower or "hi" in text_lower:
                target_lang, lang_name = "hi", "Hindi"
            elif "french" in text_lower or "fr" in text_lower:
                target_lang, lang_name = "fr", "French"
            elif "portuguese" in text_lower or "pt" in text_lower:
                target_lang, lang_name = "pt", "Portuguese"
            elif "arabic" in text_lower or "ar" in text_lower:
                target_lang, lang_name = "ar", "Arabic"
            
            if any(v_word in text_lower for v_word in ["volunteer", "staff", "roster", "people", "person", "someone"]):
                active_agent = "incident_commander"
                vols = state.get("volunteers", [])
                matched_vols = [v["name"] for v in vols if target_lang in v.get("languages", [])]
                if matched_vols:
                    response_text = f"The following safety volunteers speak {lang_name}: {', '.join(matched_vols)}."
                else:
                    response_text = f"There are currently no safety volunteers rostered who speak {lang_name}."
            else:
                active_agent = "coordinator"
                response_text = f"Yes, I speak and support {lang_name}! You can select {lang_name} from the dropdown in the header of the Fan Portal to translate the entire dashboard and hear spoken audio responses in {lang_name}."
...
async def chat(req: ChatRequest):
    # 1. CHECK QUERY CACHE
    cached_response = chat_cache.get(req.session_id, req.message)
    if cached_response:
        logger.info(f"Query cache hit for session '{req.session_id}': '{req.message}'")
        return {
            "response": cached_response,
            "state": req.state,
            "events": [
                {"author": "coordinator", "content": f"[Cache Hit] Re-routing cached response.", "actions": ""},
                {"author": "fan_assistant", "content": cached_response, "actions": ""}
            ]
        }

    # 2. EVALUATE API MODE
    if runtime_config["api_mode"] == "mock" or not runtime_config["api_key"]:
        logger.info("Mock mode active. Responding locally with high-fidelity synthetic telemetry.")
        res_text, agent = run_local_mock_routing(req.message, req.state)
        chat_cache.set(req.session_id, req.message, res_text)
        return {
            "response": res_text,
            "state": req.state,
            "events": [
                {"author": "coordinator", "content": f"Evaluating routing for request: '{req.message}'", "actions": ""},
                {"author": agent, "content": res_text, "actions": ""}
            ]
        }

    # 3. RUN LIVE ADK AGENTS (WITH SELF-HEALING FALLBACK SHIELD)
    try:
        state_delta = {
        ...
        chat_cache.set(req.session_id, req.message, bot_response)
        ...
        logger.exception("Live ADK call failed. Initiating Self-Healing fallback sequence to maintain server uptime.")
        # Fall back gracefully to mock router instead of throwing a 500 error!
        res_text, agent = run_local_mock_routing(req.message, req.state)
        chat_cache.set(req.session_id, req.message, res_text)
        return {
            "response": f"[API Offline Fallback] {res_text}",
            "state": req.state,
            "events": [
                {"author": "coordinator", "content": f"[Self-Healing] Live agent error: {str(live_err)}. Routing via Mock.", "actions": ""},
                {"author": agent, "content": res_text, "actions": ""}
            ]
        }
